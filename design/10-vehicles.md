...
Hull classes: H1 = 0.5 MPa (any Titan depth; the design driver is 94 K, not pressure), H2 = 5 MPa, H3 = 40 MPa (Europa under-ice: ice-shell base at 25 km ≈ ρ_ice·g·h = 920·1.31·25,000 ≈ **30 MPa**), H4 = abyssal Europa (100+ MPa ocean-floor traversal — **in v1 per DECISIONS C20**, T4 [SPECULATIVE]; design lands at Pass 2/Phase 5). Exceeding rating: leak events at 1%/min per 10% overpressure.
...
Vehicles use 05's condition C ∈ [0,1], MTBF, and spares-split mechanics unchanged (this closes 05's open question #5-Q "vehicles must confirm"). DECISIONS A5 ratifies the unified wear model: **02's per-ignition/wear base × 11's maturity factor × 05's spares/MTBF consumption** — orthogonal multipliers, no double counting (mounted 02 engine rows keep 02's wear model; V-24 below feeds only 05's spares/MTBF layer); ships as an interface test in Phase 1. Vehicle-specific *wear drivers* feed C:
...
| RVR-MULE | Rover-manipulator | T2 | VC-1 | 0.45 | STO-LI 15 kWh | 0 | 8/24 km/h | 0.06 | 200 km | = 05's bot_mule (single data row confirmed, owned by 05 — DECISIONS A9; 05 owns its labor math) |
...
| **T2–T3, Act 4 (Belt + Venus + Mercury)** | Venus: VH-09 balloons → variable-altitude aerobots → cloud gliders around 07's aerostats; VH-10 Venus surface crawler converts the Venera-13 trophy contract from suicide run to 60-day campaign; DECISIONS C17 adds the Venus **crewed** surface sortie as an endgame T4 [SPECULATIVE] trophy (single short-duration sortie in a cooled suit-vehicle — interface designed with 07/08 at Pass 2). Mercury: RVR-CRAWL terminator-pacing platform (07 HAB-16) — the game's slowest, coolest vehicle. Ceres: hopper-first mobility doctrine. |
...
3. **Titan human-powered flight**: shipped as a true EVA-mobility mode (this doc's current spec) or demoted to a morale/wonder event to save UI surface? Realism is not in question — scope is. **RESOLVED per DECISIONS C20: IN v1** as the true EVA-mobility mode — this doc's current spec (§3.5.6, HPF-T) ships; Phase 5 scope growth accepted knowingly.
4. **Venus crewed surface**: 03's open question — if the decision lands "robots only forever," VH-10 stays as-is; if crewed sorties are allowed at T4, this doc needs a crewed refractory crawler with an 08 interface nobody has designed. Recommend robots-only (matches realism reviewers). **DECIDED per DECISIONS C17: IN as a T4 [SPECULATIVE] trophy** — a single short-duration crewed sortie in a cooled suit-vehicle, endgame achievement tier. The robotic VH-10 line (RVR-VENUS, T3) stays as-is; the crewed suit-vehicle is a new T4 [SPECULATIVE] design item — 07/08/10 design the interface at Pass 2; honest speculation tag mandatory.
5. **Europa ocean scope in v1**: ROV-E currently terminates at science arcs near the breach point. Full ocean-floor traversal (H4 hulls, 100+ MPa) is physically specifiable but content-empty — cut or keep as post-v1? **RESOLVED per DECISIONS C20: IN v1** — full ocean-floor traversal ships (H4 hull class, §3.7.2, T4 [SPECULATIVE]); one of the two acknowledged Phase 5 heavy items, content design at Pass 2.
6. **Boats/waves**: is the sea-state 0–4 abstraction enough, or does Titan storm sailing deserve real wave mechanics? (Cassini evidence says seas are usually flat — realism argues for the abstraction.) **RESOLVED per DECISIONS C20: IN v1** — boats ship with a real wave model (the other acknowledged Phase 5 heavy item); Phase 5 scope growth accepted knowingly.
7. **Legged locomotion**: deliberately absent (no flown anchor beyond testbeds; tracked/wheeled covers v1 terrain). Revisit if CHAOS-sector content needs it; ATHLETE (JPL) would be the T3 anchor. **RESOLVED per DECISIONS C20: IN v1** — legged locomotion ships in the exotic-vehicle bundle; ATHLETE (JPL) becomes the T3 anchor, chassis/parts design lands at Pass 2/Phase 5.
8. **Shared-stat row with 05's bot_mule**: one vehicle defined in two docs (05 labor math, 10 chassis stats) — confirm at integration that both docs point at a single data row to avoid divergence (action: 05+10 joint review). **RESOLVED per DECISIONS A9: confirmed** — bot_mule is a single data row owned by 05; this doc's RVR-MULE (§4.1) points at it.
...
10. **Crewed-hopper range vs 02's lander-engine ceiling**: with 02's best lander engine at Isp 320 s vac (ML-24), HOP-P closes at 1,600 m/s tank-full (~360 km Moon) — short of a 450 km sortie class. Options: accept it (more pads, staged hops — arguably better infrastructure gameplay), or request that 02 add a pump-fed methalox *lander* row (Raptor-derived lander class, ~345 s vac plausible within 02's own pump-fed ceiling) and re-quote HOP-P against its ID. Joint owners 02 + 10; this doc will not carry an Isp that 02's catalog cannot source. **RESOLVED per DECISIONS A8: the registration was accepted** (alongside 06's Bantam-class methalox pair and argon Hall string) — the pump-fed methalox lander row now exists in 02's catalog, 02 is publishing the stats, and HOP-P re-quotes against the 02 ID when they land.
