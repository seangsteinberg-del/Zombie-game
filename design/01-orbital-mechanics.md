...
**Lambert solver (window scan).** The "Window Finder" sweeps departure/arrival date pairs and solves the 2D Lambert problem (universal-variable Lambert, BMW ch. 5, iterating on z with the same Stumpff functions as 3.3) for total Δv; result is displayed as a 1D-per-departure-date "window bar" (a porkchop collapses to clean curves in 2D) — the default presentation; the full 2-axis (departure × arrival date) porkchop heatmap is available behind an "Advanced" toggle (same solver, denser scan — DECISIONS C25). Prograde short-way solutions by default; long-way available behind the same "Advanced" toggle. Note: the classic 180°-transfer singularity of 3D Lambert (undefined transfer plane) **does not exist in 2D** — the plane is fixed; the solver is well-conditioned at all transfer angles except exact 0°/360°.
...
| Sun–Earth L1/L2 [v1 — DECISIONS C18] | 1.5e6 km sun/anti-sun | within 100,000 km AND v_rel < 50 m/s | **4 m/s/yr** auto-deducted RCS (unstable in reality) | SOHO 2.4 m/s/yr, JWST ≤ 4 m/s/yr; confirmed IN v1 (enables early science-station contracts) |
...
**Skyhook tether tips (T4 [SPECULATIVE]) — same doctrine (DECISIONS A9).** The momentum-exchange skyhook (05 `log_skyhook`; 11 SH-08) presents to this domain as a **standard orbital node with a Δv discount**, not a new flight regime. The tether hub flies an ordinary on-rails conic (propagated per 3.3); the tip is an analytic offset from the hub (rotation phase × tether length), and a catch is a standard rendezvous against the tip state — the tip's velocity at catch differs from local circular speed by the tip Δv (2.4 km/s per 05's `log_skyhook` row), and that difference *is* the discount: the climbing payload arrives 2.4 km/s short of orbital speed and the tether carries it the rest of the way. **01 provides state only**: hub elements, tip position/velocity at any t, and catch-window events in the queue (3.6). The momentum ledger — boost debt, down-mass repayment, electrodynamic/SEP reboost — is owned entirely by 05 and touches this domain only as a commanded hub-orbit change (a normal element refit). No tether dynamics (libration, flex) are simulated — honesty note as for the slots above.
...
- **Window Finder:** timeline of departure windows per target with bar height = total Δv from catalog (Table 4.2 baseline) and exact next-window countdown; clicking a window auto-creates the transfer node chain. An "Advanced" toggle reveals the full 2-axis (depart × arrive) porkchop heatmap (DECISIONS C25). Alarm clock integration (alarms pause warp).
...
- Consume: Δv map edges as transport-cost inputs; synodic windows (4.3) as logistics cadence; stationkeeping fees (4.8) as recurring RCS-propellant demand; eclipse fraction rule (3.14) for relay/depot siting; skyhook tether-tip state and catch-window events (3.13 — 05 owns the momentum ledger, 01 owns the orbital state, per DECISIONS A9).
...
1. **L1/L2 slots in v1 or v1.1?** The spec (3.13) is ready, but Sun–Earth L1/L2 mainly serve telescope/relay flavor; recommend v1.1 unless 12-gameplay wants an early-game science-station contract line. — Needs 12 decision. **RESOLVED (DECISIONS C18): IN v1** — cheap on-rails slots enabling the early science-station contract line; the [optional v1.1] tag in 3.13 is lifted.
2. **Lambert-scan UX depth:** is the 1D window-bar enough, or do players want a full 2-axis (depart × arrive) porkchop heatmap behind an "advanced" toggle? Cheap to add (same solver); UI clutter question for 12. **RESOLVED (DECISIONS C25):** 1D window bar is the default; the full 2-axis porkchop heatmap ships behind the "Advanced" toggle (same solver — 3.8 and Section 5 updated).
...
8. **N-body flag for a future "Principia mode":** the architecture (body-centric states, event queue) does not preclude swapping the rails propagator for numerical n-body later. Do we reserve save-format headroom for it (13)? Recommend yes — one schema version field. **RESOLVED (DECISIONS G35e): yes** — one reserved save-schema version field (13 §3.15's `"propagator"` field), reserved from the first shipped schema (Phase 1).
