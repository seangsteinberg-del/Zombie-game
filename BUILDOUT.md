...
| D | Drydock 2.0 | IN PROGRESS (D1 26fa7f5: full 118-part catalog w/ §5.1 schema via tools/gen_parts2.py; D2 17bf24f: sim/vessels/grid.py — w×h footprints at (x,y) +y=nose, derived stack/radial/interior joints w/ node rules incl. no_bottom engines + fairing interiors + surface-mount RCS, E1–E5, 600 cap, dry COM, persistence; D3 2ec0ddd: sim/vessels/buildermath.py — ṁ-constant-from-vac-pair law, ambient/traj-avg Isp, mixed Isp, EP power fraction vs RATED kWe + 1/d² solar, O/F burnable min-limit, bottom-up stage report; Builds A/B/C acceptance numbers pinned (canon conflict documented: catalog SL thrust rows vs ṁ-model → TWR 1.78 not 1.81); D4 6f6b47b: sim/vessels/structure.py — limiter-clamped joint loads (Build A 947 kN worked example exact), grid spanning-tree loads, q_max E7, q·α 170+fins, pre-flight ascent q-sim w/ throttle bucket. NEXT: D5 O/F drain split across connected tanks + flameout + crossfeed feed groups (FD-1/DK-L descending-stage order, W9) + Build D four-tank drain test; D6 the Drydock grid-editor UI scene — grid render/place/move, catalog pane, live readouts panel (dv/TWR/COM/badges/part count/cost), validation list w/ zoom-to-offender, ascent-sim plot panel; then port docking E8 → T chunk) | 2D grid builder, ~110-part catalog, O/F plumbing + feed groups, validation E1–E11 + W warns, pre-flight ascent sim, 4 worked builds |
